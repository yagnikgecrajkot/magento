...
class Ccc_Meet_Block_Adminhtml_Meet_Attribute_Set_Main extends Mage_Adminhtml_Block_Template
{
    
    protected function _construct()
    {
        $this->setTemplate('meet/attribute/set/main.phtml');
    }
    ...
    protected function _prepareLayout()
    {
        ...
        $this->setChild('group_tree',
            $this->getLayout()->createBlock('meet/adminhtml_meet_attribute_set_main_tree_group')
        );
        ...
        $this->setChild('delete_group_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('meet')->__('Delete Selected Group'),
        ...
        $this->setChild('add_group_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('meet')->__('Add New'),
        ...
        $this->setChild('reset_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('meet')->__('Reset'),
                'onclick'   => 'window.location.reload()'
        )));

        $this->setChild('save_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('meet')->__('Save Attribute Set'),
                'onclick'   => 'editSet.save();',
                'class'     => 'save'
        )));

        $deleteConfirmMessage = $this->jsQuoteEscape(Mage::helper('meet')
            ->__('All products of this set will be deleted! Are you sure you want to delete this attribute set?'));
        $deleteUrl = $this->getUrl('*/adminhtml_meet_set/delete', array('id' => $setId));

        $this->setChild('delete_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('meet')->__('Delete Attribute Set'),
        ...
        $this->setChild('rename_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('meet')->__('New Set Name'),
    ...
    {
        return Mage::helper('meet')->__("Edit Attribute Set '%s'", $this->_getAttributeSet()->getAttributeSetName());
    }

    public function getMoveUrl()
    {
        return $this->getUrl('*/adminhtml_meet_set/save', array('id' => $this->_getSetId()));
    }

    public function getGroupUrl()
    {
        return $this->getUrl('*/adminhtml_meet_group/save', array('id' => $this->_getSetId()));
    }
    ...
    public function getGroupTreeJson()
    {
        ...
        foreach ($groups as $node) {
            ...
            $nodeChildren = Mage::getResourceModel('meet/meet_attribute_collection')
    ...
    public function getAttributeTreeJson()
    {
        ...
        $collection = Mage::getResourceModel('meet/meet_attribute_collection')
            ->setAttributeSetFilter($setId)
            ->load();
        ...
        $attributes = Mage::getResourceModel('meet/meet_attribute_collection')
        ...
        if (count($items) == 0) {
            $items[] = array(
                'text'      => Mage::helper('meet')->__('Empty'),
                'id'        => 'empty',
                'cls'       => 'folder',
                'allowDrop' => false,
                'allowDrag' => false,
